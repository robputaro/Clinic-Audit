...
// Vercel serves files in public/ directly. Local development uses this fallback.
app.use("/assets",express.static(path.join(__dirname,"public","assets"),{fallthrough:false,maxAge:"1h"}));
...
function validateIdentity(body){
  if(!String(body.clinicName||"").trim())throw new Error("Clinic name is required");
  if(!String(body.city||"").trim())throw new Error("City is required");
  if(!["yes","no","unsure"].includes(body.financeStatus))throw new Error("Finance status is required");
  if(!Array.isArray(body.devices)||body.devices.length<1)throw new Error("Select at least one device");
  if(body.privacyConsent!==true)throw new Error("Privacy acknowledgment is required");
  if(body.anonymousReview&&!body.liveReviewConsent)throw new Error("Anonymous review requires live-review consent");
}

app.get("/",(req,res)=>res.redirect("/audit"));
app.get("/audit",(req,res)=>res.sendFile(path.join(viewsDir,"audit.html")));
app.get("/stage",(req,res)=>res.sendFile(path.join(viewsDir,validSession(req)?"stage.html":"stage-login.html")));
app.get("/stage/projector/:id",(req,res)=>res.sendFile(path.join(viewsDir,validSession(req)?"projector.html":"stage-login.html")));
...
app.post("/api/submit",async(req,res)=>{
  try{
    ...
    const q=`INSERT INTO audit_submissions (id,clinic_name,city,website,instagram,devices,finance_status,provider_count,live_review_consent,anonymous_review,privacy_consent,privacy_notice_version,answers,score,blind_spot,unknown_count,burn_flag) VALUES ($1,$2,$3,$4,$5,$6::jsonb,$7,$8,$9,$10,$11,$12,$13::jsonb,$14,$15,$16,$17)`;
    await pool.query(q,[id,req.body.clinicName.trim(),req.body.city.trim(),req.body.website||null,req.body.instagram||null,JSON.stringify(req.body.devices),req.body.financeStatus,req.body.providerCount||null,!!req.body.liveReviewConsent,!!req.body.anonymousReview,true,"2026-07-14",JSON.stringify(req.body.answers),scored.total,scored.blind,scored.unknowns,scored.burn]);
...
app.delete("/api/submission/:id",requireStage,async(req,res)=>{
  try{
    const {rowCount}=await pool.query("DELETE FROM audit_submissions WHERE id=$1",[req.params.id]);
    if(!rowCount)return res.status(404).json({error:"Submission not found"});
    res.json({ok:true});
  }catch(err){console.error(err);res.status(500).json({error:"Could not delete submission"});}
});
